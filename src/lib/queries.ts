...
export type ViewerBooking = DashboardBooking & { professional: PublicProfile; customerConfirmedAt: Date | null; review?: { rating: number; comment: string } | null };
...
async function paginatedTable(name: string, page = 1, pageSize = 25): Promise<Paginated<Row>> {
  const supabase = createSupabasePublicClient();
  const from = Math.max(0, page - 1) * pageSize;
  const to = from + pageSize - 1;
  const { data, count, error } = await supabase.from(name).select("*", { count: "exact" }).range(from, to);
  if (error) throw new Error(`Supabase query failed: ${error.message}`);
  return { items: (data ?? []) as unknown as Row[], total: count ?? 0, page, pageSize };
}
...
export const getPublicCompaniesPage = (page = 1) => paginatedTable("public_company_cards", page);
export const getPublicProfessionsPage = (page = 1) => paginatedTable("public_profession_cards", page);
...
export async function searchProfessionalsPage(params: { q?: string; company?: string; profession?: string; mode?: string; page?: number }): Promise<Paginated<Row>> { const supabase = createSupabasePublicClient(); const page = params.page ?? 1; const pageSize = 25; const from = (page - 1) * pageSize; let request = supabase.from("public_profile_cards").select("*", { count: "exact" }); if (params.mode) request = request.eq("workMode", params.mode); const { data, count, error } = await request.range(from, from + pageSize - 1); if (error) throw new Error(`Supabase query failed: ${error.message}`); const q = params.q?.toLowerCase(); const items = ((data ?? []) as unknown as Row[]).filter((p) => !q || JSON.stringify(p).toLowerCase().includes(q)); return { items, total: count ?? 0, page, pageSize }; }
...
export async function getConsultantDashboard(userId: string): Promise<ConsultantDashboard | null> { const supabase = await createSupabaseServerClient(); const { data, error } = await supabase.rpc("get_consultant_dashboard", { p_user_id: userId }); if (error) throw new Error(`Supabase consultant dashboard query failed: ${error.message}`); if (!data) return null; const dashboard = data as ConsultantDashboard; dashboard.availability = Array.isArray(dashboard.availability) ? dashboard.availability.map((slot) => ({ ...slot, startsAt: new Date(slot.startsAt), endsAt: new Date(slot.endsAt) })).filter((slot) => !Number.isNaN(slot.startsAt.getTime()) && !Number.isNaN(slot.endsAt.getTime())) : []; dashboard.bookings = Array.isArray(dashboard.bookings) ? dashboard.bookings.map((booking) => ({ ...booking, startsAt: new Date(booking.startsAt), consultantConfirmedAt: booking.consultantConfirmedAt ? new Date(booking.consultantConfirmedAt) : null })) : []; return dashboard; }
