...
export function TwoFactorChallenge() {
  ...
  return <form action={action} className={`auth-form form-stack mfa-challenge ${phase === "error" ? "mfa-error" : ""} ${phase === "success" ? "mfa-success" : ""}`}><span className="eyebrow">Verificação de segurança</span><h2>Confirme seu Authenticator.</h2><p className="muted">Digite o código de seis números exibido no Google Authenticator para continuar.</p><input type="hidden" name="code" value={code}/><div className="mfa-code-boxes" role="group" aria-label="Código de seis números">{Array.from({ length: 6 }, (_, index) => <input key={index} ref={(element) => { inputs.current[index] = element; }} className="mfa-code-box" aria-label={`Dígito ${index + 1} de 6`} inputMode="numeric" maxLength={1} value={code[index] ?? ""} onChange={(event) => update(index, event.target.value)} onPaste={paste} onKeyDown={(event) => keyDown(index, event)} autoFocus={index === 0}/>)}</div>{phase === "success" && <div className="mfa-confirmation" aria-live="polite"><Check size={24}/> Código confirmado</div>}{state?.status === "error" && <p className="form-error" role="alert">{state.message}</p>}<button className="button button-accent button-block" disabled={pending || code.length !== 6}>{pending ? "Verificando…" : "Confirmar código"}</button></form>;
}
