...
type PanelName = "empresas" | "profissoes" | "reality" | null;
...
export function PremiumNavigation({ dashboardHref }: { dashboardHref: string | null }) {
  ...
  return <div className="header-wrap"><header className="header premium-header" aria-label="Navegação principal">
    <Link className="brand premium-brand" href="/#inicio" onClick={goHome} aria-label="Insidely — início">inside<span>ly</span><i>.</i></Link>
    <nav className="premium-nav" aria-label="Seções do site"><Link href="/buscar">Profissionais</Link>{(Object.keys(panels) as Exclude<PanelName, null>[]).map((key) => <button type="button" key={key} aria-expanded={activeName === key} onClick={() => setActivePanel(activeName === key ? null : { name: key, path: pathname })}>{panels[key].label}<ChevronDown size={14}/></button>)}</nav>
  ...
  </header>{mobileOpen && <nav className="mobile-drawer" aria-label="Navegação móvel"><Link href="/buscar">Profissionais</Link><Link href="/empresas">Empresas</Link><Link href="/profissoes">Profissões</Link><Link href="/reality-check">Reality Check</Link>{dashboardHref ? <Link href={dashboardHref}>Meu painel</Link> : <><Link href="/entrar">Entrar</Link><Link href="/cadastro">Criar conta</Link></>}</nav>}</div>;
}
