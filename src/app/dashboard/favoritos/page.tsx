...
export default async function Page() {
  ...
  return <DashboardShell mode="user" title="Favoritos" canConsultant={user.role === Role.CONSULTANT}>
    <section>
      ...
      <div className="grid grid-3">{favorites.map((favorite, index) => <div className="favorite-profile-item" key={favorite.id}><ProfessionalCard profile={favorite.professionalProfile} index={index} /><div className="favorite-card-actions"><FavoriteButton profileId={favorite.professionalProfile.id} initialSaved showShare={false} /><ProfileShareButton profileId={favorite.professionalProfile.id} /></div></div>)}</div>
