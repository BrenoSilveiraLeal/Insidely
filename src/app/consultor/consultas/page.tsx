...
function stateLabel(status: string) { return ({ CONFIRMED: "Confirmada Â· valor retido", AWAITING_CONFIRMATION: "Aguardando confirmaÃ§Ã£o", COMPLETED: "ConcluÃ­da Â· repasse liberado", DISPUTED: "Em anÃ¡lise pelo suporte" } as Record<string, string>)[status] ?? status; }
...
export default async function ConsultantConsultationsPage() {
  ...
  return <DashboardShell mode="consultant" title="Consultas"><div className="grid grid-2">{profile?.bookings.map((booking) => <article className="panel" key={booking.id}><span className="eyebrow">{stateLabel(booking.status)} Â· {shortDate(booking.startsAt)}</span><h2>{booking.customer.name}</h2><p>{booking.goals}</p>
