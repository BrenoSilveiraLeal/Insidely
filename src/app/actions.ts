...
export async function markNotificationReadAction(id: string) { const user = await requireUser(); const supabase = await createSupabaseServerClient(); await supabase.from("Notification").update({ readAt: new Date().toISOString() }).eq("id", id).eq("userId", user.id).is("readAt", null); revalidatePath("/dashboard"); }
export async function markAllNotificationsReadAction() { const user = await requireUser(); const supabase = await createSupabaseServerClient(); await supabase.from("Notification").update({ readAt: new Date().toISOString() }).eq("userId", user.id).is("readAt", null); revalidatePath("/dashboard"); }
