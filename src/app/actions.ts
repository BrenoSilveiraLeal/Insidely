...
export async function payBookingAction(id: string, formData: FormData) { await requireUser(); if (formData.get("recordingConsent") !== "on") throw new Error("Confirme as regras antes de continuar."); await rpc("hold_booking_payment", { p_booking_id: id, p_method: String(formData.get("paymentMethod") || "PIX") }); redirect("/dashboard/agendamentos?confirmado=1"); }
...
export async function deleteAccountAction(formData: FormData) { await requireUser(); const supabase = await createSupabaseServerClient(); const { error } = await supabase.functions.invoke("delete-account", { body: { confirmation: String(formData.get("confirmation") || "") } }); if (error) throw new Error("N foi poss concluir a remo??o segura da conta."); await supabase.auth.signOut(); redirect("/?conta=excluida"); }
...
export async function verifySupabaseTwoFactorAction(_: FormState, formData: FormData): Promise<FormState> { const code = String(formData.get("code") || "").replace(/\D/g, ""); if (!/^\d{6}$/.test(code)) return { status: "error", message: "Digite o c." }; const supabase = await createSupabaseServerClient(); const { data: factors, error: listError } = await supabase.auth.mfa.listFactors(); const factor = factors?.totp.find((item) => item.status === "verified"); if (listError || !factor) return { status: "error", message: "Nenhum Authenticator ativo foi encontrado." }; const { data: challenge, error: challengeError } = await supabase.auth.mfa.challenge({ factorId: factor.id }); if (challengeError || !challenge) return { status: "error", message: "N foi poss iniciar a verifica." }; const { error } = await supabase.auth.mfa.verify({ factorId: factor.id, challengeId: challenge.id, code }); if (error) return { status: "error", message: "C inv. Tente novamente." }; redirect("/continuar"); }
export async function enableSupabaseTwoFactorAction(_: FormState, formData: FormData): Promise<FormState> { const code = String(formData.get("code") || "").replace(/\D/g, ""); const factorId = String(formData.get("factorId") || ""); if (!/^\d{6}$/.test(code) || !factorId) return { status: "error", message: "Digite o c." }; const supabase = await createSupabaseServerClient(); const { data: challenge, error: challengeError } = await supabase.auth.mfa.challenge({ factorId }); if (challengeError || !challenge) return { status: "error", message: "N foi poss iniciar a verifica." }; const { error } = await supabase.auth.mfa.verify({ factorId, challengeId: challenge.id, code }); return error ? { status: "error", message: "C inv ou expirado." } : { status: "success", message: "Prote ativada." }; }
export async function disableSupabaseTwoFactorAction(_: FormState, formData: FormData): Promise<FormState> { const code = String(formData.get("code") || "").replace(/\D/g, ""); if (!/^\d{6}$/.test(code)) return { status: "error", message: "Digite o c." }; const supabase = await createSupabaseServerClient(); const { data: factors } = await supabase.auth.mfa.listFactors(); const factor = factors?.totp.find((item) => item.status === "verified"); if (!factor) return { status: "error", message: "Nenhum Authenticator ativo foi encontrado." }; const { data: challenge, error: challengeError } = await supabase.auth.mfa.challenge({ factorId: factor.id }); if (challengeError || !challenge) return { status: "error", message: "N foi poss iniciar a verifica." }; const { error: verifyError } = await supabase.auth.mfa.verify({ factorId: factor.id, challengeId: challenge.id, code }); if (verifyError) return { status: "error", message: "C inv. Tente novamente." }; const { error } = await supabase.auth.mfa.unenroll({ factorId: factor.id }); return error ? { status: "error", message: "N foi poss desativar a prote." } : { status: "success", message: "Prote desativada." }; }
export async function getOrCreateTwoFactorSetup() { return null; }
export async function enableTwoFactorAction(): Promise<FormState> { return { status: "error", message: "A autentica multifator ? gerenciada pelo Supabase Auth." }; }
export async function disableTwoFactorAction(): Promise<FormState> { return { status: "error", message: "A autentica multifator ? gerenciada pelo Supabase Auth." }; }
