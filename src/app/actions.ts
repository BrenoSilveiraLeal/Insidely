...
const adminAction = async (action: string, resourceId: string) => {
  ...
  if (error) throw new Error("A ação administrativa não pôde ser concluída.");
};
...
export async function submitVerificationAction(_: string | undefined, formData: FormData) { await requireUser([Role.CONSULTANT]); const file = formData.get("document"); if (!(file instanceof File)) return "Anexe um comprovante."; const supabase = await createSupabaseServerClient(); const path = `verificacoes/${crypto.randomUUID()}-${file.name}`; const upload = await supabase.storage.from("verification-documents").upload(path, file, { upsert: false }); if (upload.error) return "Não foi possível enviar o documento."; await rpc("submit_verification", { p_storage_key: path, p_original_name: file.name, p_mime_type: file.type, p_size_bytes: file.size, p_method: String(formData.get("method") || "company_email") }); revalidatePath("/consultor/perfil"); return "Solicitação enviada com segurança."; }
export async function updateProfileImageAction(_: string | undefined, formData: FormData) { const user = await requireUser(); const file = formData.get("image"); if (!(file instanceof File)) return "Escolha uma foto."; const supabase = await createSupabaseServerClient(); const path = `avatars/${user.id}/${crypto.randomUUID()}-${file.name}`; const upload = await supabase.storage.from("avatars").upload(path, file, { upsert: true }); if (upload.error) return "Não foi possível enviar a foto."; const { data } = supabase.storage.from("avatars").getPublicUrl(path); await rpc("update_profile_image", { p_image: data.publicUrl }); revalidatePath("/dashboard/configuracoes"); return "Foto atualizada."; }
