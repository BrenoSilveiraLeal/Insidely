...
if (!serviceKey) failures.push("SUPABASE_SERVICE_ROLE_KEY não foi definida.");
if (serviceKey?.startsWith("sb_publishable_")) failures.push("Foi informada a publishable key no lugar da service role key.");
if (serviceKey && /^(SUA_CHAVE|your-|replace_|placeholder|changeme)/i.test(serviceKey)) failures.push("SUPABASE_SERVICE_ROLE_KEY ainda é um placeholder.");
if (!url) failures.push("NEXT_PUBLIC_SUPABASE_URL não foi definida. Configure a URL HTTPS do projeto Insidely.");
if (url && /^[a-z0-9]{20}$/.test(url)) {
  console.log(`[E2E PREFLIGHT] Project Ref detectado; usando https://${url}.supabase.co.`);
} else if (url) {
  try {
    const parsed = new URL(url);
    if (!/^https?:$/.test(parsed.protocol)) failures.push("NEXT_PUBLIC_SUPABASE_URL precisa começar com http:// ou https://.");
  } catch {
    failures.push("NEXT_PUBLIC_SUPABASE_URL não é uma URL válida.");
  }
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401 || response.status === 403) {
        failures.push("A SUPABASE_SERVICE_ROLE_KEY foi rejeitada pelo Supabase. Confirme que URL e chave pertencem ao projeto Insidely, que a chave foi copiada integralmente e que não foi revogada.");
        ...
        failures.push("Não foi possível conectar ao Supabase. Confira NEXT_PUBLIC_SUPABASE_URL e a rede.");
  }
}
