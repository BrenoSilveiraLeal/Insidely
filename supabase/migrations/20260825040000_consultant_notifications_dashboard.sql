-- Reuse the existing Notification table in the consultant dashboard.
...
begin
  ...
  select jsonb_build_object(
    'id',v_profile.id,'userId',v_profile."userId",'headline',v_profile.headline,'bio',v_profile.bio,
    'publicSurname',v_profile."publicSurname",'location',v_profile.location,'region',v_profile.region,
    'workMode',v_profile."workMode",'seniority',v_profile.seniority,'yearsExperience',v_profile."yearsExperience",
    'price30Cents',v_profile."price30Cents",'price60Cents',v_profile."price60Cents",'pixKey',v_profile."pixKey",
    'responseHours',v_profile."responseHours",'privacyMode',v_profile."privacyMode",'pseudonym',v_profile.pseudonym,
    'avatarSeed',v_profile."avatarSeed",'topics',v_profile.topics,'boundaries',v_profile.boundaries,
    'verificationStatus',v_profile."verificationStatus",'isActive',v_profile."isActive",'createdAt',v_profile."createdAt",'updatedAt',v_profile."updatedAt",
    ...
    'experiences',coalesce((select jsonb_agg(to_jsonb(x)||jsonb_build_object('company',(select to_jsonb(c) from public."Company" c where c.id=x."companyId"),'profession',(select to_jsonb(f) from public."Profession" f where f.id=x."professionId")) order by x."isCurrent" desc) from public."EmploymentExperience" x where x."professionalProfileId"=v_profile.id),'[]'::jsonb),
    ...
    'favorites',coalesce((select jsonb_agg(to_jsonb(f)) from public."Favorite" f where f."userId"=p_user_id),'[]'::jsonb),
    'notifications',coalesce((select jsonb_agg(to_jsonb(n) order by n."createdAt" desc) from public."Notification" n where n."userId"=p_user_id),'[]'::jsonb),
